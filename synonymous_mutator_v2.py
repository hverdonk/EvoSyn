...
parser.add_argument('--flank_length', type=int, default=250, help='Length of flanking region in base pairs')
...
left_flank = species_seq[:flank_len]
right_flank = species_seq[-flank_len:]
coding_seq = species_seq[flank_len:-flank_len]

# Ensure coding region is divisible by 3
if len(coding_seq) % 3 != 0:
    raise ValueError("Coding region length is not divisible by 3 (not in-frame).")
...
output_records.append(SeqIO.SeqRecord(Seq(left_flank + "".join(codons) + right_flank), id=cleaned_species_id + "_original", description=""))
